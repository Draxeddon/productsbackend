...
const cartItemSchema = z.object({
    productId: z.string('El id del producto es requerido').min(1, {error: 'El id del producto no es valido'}),
    quantity: z.string().transform((val) => parseInt(val)).pipe(z.number('Cantidad de producto requerido')
                                                                    .positive('Cantidad debe ser mayor a 0')
                                                                    .refine((val) => !isNaN(val), {error: 'Cantidad debe ser un numero valido'})),
    price: z.string().transform((val) => parseFloat(val)).pipe(z.number()
                                                                .int({error: 'Precio del producto requerido'})
...
const cardDetailSchema = z.object({
    cardName: z.string('El nombre de la tarjeta es requerido').min(3, {error: 'El nombre de la tarjeta debe tener al menos 3 caracteres'}).trim(),
    cardNumber: z.string('Numero de tarjeta requerido').min(12, {error: 'El numero de tarjeta debe tener al menos 12 digitos'})
                                                        .max(19, {error: 'El numero de tarjeta no puede exceder de 19 digitos'})
                                                        .regex(/^\d{12,19}$/, 'Numero de tarjeta invalido')
                                                        .trim(),
    ccv: z.string('CCV requerido').min(3, {error: 'El CCV debe tener la menos 3 digitos'})
                                   .max(4, {error: 'El CCV no puede exceder 4 digitos'})
                                   .regex(/^\d{3,4}$/, 'CCV invalido')
                                   .trim(),
    expirationDate: z.string('Fecha de expiracion requerida').regex(/^(0[1-9]|1[0-2])\/?([0-9]{2})$/, 'Formato de fecha invalido (MM/YY)').trim(),                 
});

//esquema para la direccion de envio
const shippingAddressSchema = z.object({
    address: z.string('La direccion es requerida').min(5, {error: 'La direccion debe tener al menos 3 caracetres'}).trim(),
    name: z.string('El nombre es requerido').min(3, {error: 'El nombre debe tener al menos 3 caracteres'}).trim(),
    phone: z.string('El telefono es requerido').min(7, {error: 'El telefono debe tener al menos 7 caracteres'})
                                                .max(20, {error: 'El telefono no puede exceder 20 caracteres'})
...
const paymentMethodSchema = z.discriminatedUnion('method',
    [
        ...
        z.object({
            ...
            userName: z.string('El nombre es requerido').min(3, {error: 'El nombre debe tener al menos 3 caracteres'}).trim(),
        })
    ]
);
...
export const orderSchema = z.object({
    items: z.array(cartItemSchema).min(1, 'La orden debe tener al menos un producto'),
    ...
    subTotal: z.string().transform((val) => parseFloat(val)).pipe(
    ...
    iva: z.string().transform((val) => parseFloat(val)).pipe(
                                                        z.number('IVA requerido')
                                                    .min(0, {error: 'IVA debe ser mayor o igual a 0'})
                                                .refine((val) => !isNaN(val), {error: 'IVA debe ser un numero valido'})),
    total: z.string().transform((val) => parseFloat(val)).pipe(
                                                        z.number('Total del pedido requerido')
                                                    .min(1, {error: 'El total del pedido debe ser mayor a cero'})
                                                .refine((val) => !isNaN(val), {error: 'Total del pedido debe ser un numero valido'})),
    totalProducts: z.string().transform((val) => parseFloat(val)).pipe(
                                                        z.number().int({error: 'Total de productos requerido'})
    ...
    //datos para el estado del pedido y la fecha y hora de creacion
